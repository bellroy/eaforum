...
class UserController(ListingController):
    ...
    def builder(self):
        if self.where in ('comments', 'overview', 'submitted'):
            ...
            b = builder_cls(self.query_obj,
                             ...
                             reverse = self.reverse,
                             wrap = self.builder_wrapper,
                             sr_ids = [c.current_or_default_sr._id, Subreddit._by_name('meetups')._id])
            return b
        else:
    ...
    def GET_listing(self, where, vuser, **env):
        ...
        if (where not in ('overview', 'submitted', 'comments', 'overviewrss', 'commentsrss')
